...
class Point(object):
    ...
    def move_to(self, x, y):
        ...
        self.travel = self.travel + math.sqrt((x - self.__init__x) ** 2 + (y - self.__init__y) ** 2)

    def move_by(self, x, y):
        self.x = self.x + x
        self.y = self.y + y
        self.move = self.move + 1
        self.travel = self.travel + math.sqrt(x ** 2 + y**2)
    ...
    def get_distance_from_start(self):
        distance = math.sqrt((self.x - self.__init__x)**2 + (self.y - self.__init__y)**2)
        return distance

    def closer_to(self, p2, p3):
        distance1 = Point.get_distance_from(self, p2)
        distance2 = Point.get_distance_from(self, p3)
